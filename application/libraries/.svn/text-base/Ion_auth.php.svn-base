...
class Ion_auth {
     ...
     * @author Mathew
     * */
    public function register($password, $email, $additional_data, $group_name = false) { //need to test email activation
        $email_activation = $this->ci->config->item('email_activation', 'ion_auth');

        if (!$email_activation) {
            $id = $this->ci->ion_auth_model->register($password, $email, $additional_data, $group_name);
            if ($id !== FALSE) {
     ...
     * @author Ben Edmunds
     * */
    public function get_users($group_name=false, $limit=NULL, $offset=NULL, $is_array = false) {
        $data = $this->ci->ion_auth_model->get_users($group_name, $limit, $offset);
        if ($is_array)
